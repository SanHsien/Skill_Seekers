...
def test_readme_pair_cross_links_and_names_the_fork() -> None:
    ...
    assert "yusufkaraaslan/Skill_Seekers" in zh
    assert "yusufkaraaslan/Skill_Seekers" in en
...
def test_baseline_file_is_valid_and_complete() -> None:
    ...
    assert baseline["repo"] == "https://github.com/yusufkaraaslan/Skill_Seekers.git"
    assert baseline["branch"] == "main"
    assert len(baseline["reviewed_through"]) == 40
    assert baseline["reviewed_through"] == "c333a379bfb43f42eb605fd575be955762a9fa7b"
...
def test_baseline_matches_decisions_record() -> None:
    ...
    assert "c333a37" in upstream
    assert "yusufkaraaslan/Skill_Seekers" in decisions
