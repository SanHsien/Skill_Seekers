...
USER_AGENT = "skill-seekers-dependency-freshness"
...
def is_newer_version(latest: str, minimum: str) -> bool:
    latest_key = release_key(latest)
    minimum_key = release_key(minimum)
    if not latest_key or not minimum_key:
        return False
    length = min(len(latest_key), len(minimum_key))
    return latest_key[:length] > minimum_key[:length]
