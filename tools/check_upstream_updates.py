#!/usr/bin/env python3
"""Check whether upstream has added commits, pull requests, or issues past the baseline.

The upstream baseline is stored in `tools/upstream_baseline.json`:
- `reviewed_through`: full 40-character commit SHA that was last reviewed.
- `reviewed_pr_through`: integer watermark of the highest reviewed pull request.
- `reviewed_issue_through`: integer watermark of the highest reviewed issue.
...
class UpstreamCheckError(RuntimeError):
    """Raised when the check cannot complete."""


def load_baseline(path: Path = BASELINE_PATH) -> dict:
    if not path.is_file():
        raise UpstreamCheckError(f"baseline file not found: {path}")
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise UpstreamCheckError(f"invalid JSON in baseline file: {exc}") from exc

    for key in ("repo", "branch", "reviewed_through", "reviewed_date"):
        if key not in data or not str(data[key]).strip():
            raise UpstreamCheckError(f"baseline missing required key: {key}")
    if len(str(data["reviewed_through"]).strip()) != 40:
        raise UpstreamCheckError("reviewed_through must be a 40-character commit SHA")
    return data
